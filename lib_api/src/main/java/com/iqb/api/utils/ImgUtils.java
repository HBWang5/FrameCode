...
//      ★★★★★★★★★★★★★★重点★★★★★★★★★★★★★
        ...
        String mDir = Environment.getExternalStorageDirectory() + "/IQB";
        ...
        Log.d("ImgUtils", fileName + ".jpg");
        if (!file.exists()) {
            try {
                file.createNewFile();//文件不存在，则创建文件
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
        try {
            ...
            Log.d("ImgUtils", "file.length():" + file.length());
            out.flush();
            out.close();
    ...
    public static void saveBitmap(Bitmap bitmap, String fileName, OnSaveSuccessListener onSaveSuccessListener) {
