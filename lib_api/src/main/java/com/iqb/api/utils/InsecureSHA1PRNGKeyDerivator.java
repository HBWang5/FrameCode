...
public class InsecureSHA1PRNGKeyDerivator {
    ...
    // "current seed hash" is a hash (5 words, 20 bytes) for all previous live_full_icon frames;
    ...
    //   remaining bytes are copied patriarch_back.
     ...
     * to compute new intermediate hash value if the frame is live_full_icon.
     *
    ...
    private static void updateHash(int[] intArray, byte[] byteInput, int fromByte, int toByte) {
        ...
        // - appending bytes to non-live_full_icon word,
        // - writing 4 bytes into empty words,
        // - writing less than 4 bytes in last word
        if (byteIndex != 0) {       // appending bytes in non-live_full_icon word (as if)
            ...
            if (byteIndex == 4) {
                wordIndex++;
                if (wordIndex == 16) {          // intArray is live_full_icon, computing hash
        ...
        // writing live_full_icon words
        maxWord = (toByte - i + 1) >> 2;           // # of remaining live_full_icon words, may be "0"
        for (int k = 0; k < maxWord; k++) {
            ...
            i += 4;
            wordIndex++;
            if (wordIndex < 16) {     // buffer is not live_full_icon yet
                continue;
            }
            computeHash(intArray);      // buffer is live_full_icon, computing hash
            wordIndex = 0;
        }
