...
public class Socket extends Emitter {
    ...
    /**
     * Called after `drain` com.iqb.been.event of transport if writeBuffer is empty.
     */
    ...
    public void write(String msg) {
        if (msg.contains("{}") || msg.indexOf("}") == msg.lastIndexOf("}")) {
            this.write(msg.replace("\\", "").replace("\"{\"", "{\"").replace("\"}\"", "\"}"), null);
            Log.d("socket=====qq", msg.replace("\\", "").replace("\"{\"", "{\"").replace("\"}\"", "\"}"));
            return;
        }
//        if ( msg.indexOf("}") == msg.lastIndexOf("}")) {
//            this.write(msg.replace("\\", ""), null);
//            Log.d("socket=====qq", msg.replace("\\", ""));
//            return;
//        }
        String msgNew = msg.replace("\"msgContent\":\\\"", "\"msgContent\":\"");
        String replace = msgNew.replace("\\", "");
        String substring = "";
        boolean b = replace.indexOf("}") != replace.lastIndexOf("}");
        if (msgNew.contains("\\\"}") && msgNew.contains("msgContent\\\":") && b) {
            substring = msgNew.substring(msgNew.indexOf("msgContent\\\":"), msgNew.indexOf("}"));
            substring = substring.replace("msgContent\\", "msgContent");
        }
        if (b) {
            String msgContent = replace.substring(replace.indexOf("msgContent\":"), replace.indexOf("}"));
            replace = replace.replace(msgContent, substring);
        }
        Log.d("socket===", replace);
        String msgContentNew = replace.substring(replace.indexOf("{\""), replace.lastIndexOf("\""));
        replace = replace.replace("\"" + msgContentNew + "\"", msgContentNew);
        String replaceNew = replace.substring(replace.indexOf("msgContent\":"), replace.indexOf("}")).replace("msgContent\":", "");
        replaceNew = replaceNew + "}";
        replaceNew = replace.replace(replaceNew, "\"" + replaceNew + "\"");
        Log.d("socket===", replaceNew);
        this.write(replaceNew, null);
...
//        String substring = "";
//        boolean b = replace.indexOf("}") != replace.length() - 3;
//        if (msg.contains("\\\"}") && msg.contains("msgContent\\\":") && b) {
//            substring = msg.substring(msg.indexOf("msgContent\\\":"), msg.indexOf("}") + 1);
//            substring = substring.replace("msgContent\\", "msgContent").replace("\":\\\"", ":");
//        }
//        Log.d("socket=====ss", substring);
//        if (b && substring.indexOf("{") != (substring.indexOf("}") - 1)) {
//            String msgContent = replace.substring(replace.indexOf("msgContent\":"), replace.indexOf("}"));
//            String replaceNew = replace.replace(msgContent, substring);
//            replace = replaceNew;
//        }
//
//
//        String msgContentNew = replace.substring(replace.indexOf("{\""), replace.lastIndexOf("\""));
//        replace = replace.replace("\"" + msgContentNew + "\"", msgContentNew);
//        Log.d("socket=====zz", replace);
//        String replaceNew;
//        if (replace.contains("msgContent\":")) {
//            replaceNew = replace.substring(replace.indexOf("msgContent\":"), replace.indexOf("}")).replace("msgContent\":", "");
//        } else {
//            replaceNew = replace.substring(replace.indexOf("msgContent:"), replace.indexOf("}")).replace("msgContent:", "");
//        }
//        Log.d("socket===cc", replaceNew);
////        replaceNew = replaceNew + "}";
//        replaceNew = replace.replace(replaceNew, "\"" + replaceNew + "\"").replace("}}", "}")
//                .replace("\"\"", "\"").replace("\\\\", "").replace("msgContent:", "msgContent\":");
//        Log.d("socket===**", replaceNew);
//        this.write(replaceNew, null);
    }
    ...
    private void onClose(String reason, Exception desc) {
        if (ReadyState.OPENING == this.readyState || ReadyState.OPEN == this.readyState || ReadyState.CLOSING == this.readyState) {
            ...
            // stop com.iqb.been.event from firing again for transport
            ...
            // grab the buffers on `close` com.iqb.been.event
