...
public class Socket extends Emitter {
    ...
    /**
     * Emits an com.iqb.been.event. When you pass {@link Ack} at the last argument, then the acknowledge is done.
     *
    ...
    /**
     * Emits an com.iqb.been.event with an acknowledge.
     *
     * @param event an com.iqb.been.event name
     * @param args data to send.
    ...
    private void onevent(Packet<JSONArray> packet) {
        ...
        if (logger.isLoggable(Level.FINE)) {
            logger.fine(String.format("emitting com.iqb.been.event %s", args));
        }

        if (packet.id >= 0) {
            logger.fine("attaching ack callback to com.iqb.been.event");
